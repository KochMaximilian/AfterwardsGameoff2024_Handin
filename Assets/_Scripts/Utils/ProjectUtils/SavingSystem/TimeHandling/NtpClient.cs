...
public class NtpClient : IDisposable
{
    ...
    /// <summary>
    /// Asynchronously retrieves the current network time from the NTP server.
    /// </summary>
    /// <returns>A task that represents the asynchronous operation. The task result contains the current network time.</returns>
    /// <exception cref="InvalidOperationException">Thrown when failed to retrieve network time.</exception>
    public async Task<DateTime> GetNetworkTimeAsync()
    {
        try
        {
            ...
            // Retrieve the IP addresses associated with the specified NTP server
            IPHostEntry hostEntry = await Dns.GetHostEntryAsync(_server);

            // Create an endpoint representing the NTP server
            IPEndPoint ipEndPoint = new IPEndPoint(hostEntry.AddressList[0], 123); // Port 123 is the standard NTP port
            ...
            // Send the NTP request packet to the server
            await _socket.SendAsync(new ArraySegment<byte>(ntpData), SocketFlags.None);

            // Receive the NTP response packet from the server
            await _socket.ReceiveAsync(new ArraySegment<byte>(ntpData), SocketFlags.None);
