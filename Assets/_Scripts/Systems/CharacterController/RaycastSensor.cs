...
namespace AdvancedController {
    public class RaycastSensor {
        ...
        public void Cast(Vector3 offset)
        {
            Vector3 worldOrigin = tr.TransformPoint(origin) + offset;
            Vector3 worldDirection = GetCastDirection();
            
            Physics.Raycast(worldOrigin, worldDirection, out hitInfo, castLength, layermask, QueryTriggerInteraction.Ignore);
        }

        public void Cast(float radius)
        {
            Vector3 worldOrigin = tr.TransformPoint(origin);
            Vector3 worldDirection = GetCastDirection();
            
            Physics.SphereCast(worldOrigin, radius, worldDirection, out hitInfo, castLength, layermask, QueryTriggerInteraction.Ignore);
        }
